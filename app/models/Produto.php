...
class Produto {
    ...
    public static function create($db, $data) {
        $stmt = $db->prepare('INSERT INTO produtos (nome, descricao, preco, imagem, categoria_id, destaque, tipo_estoque, estoque) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
        $stmt->execute([
            $data['nome'],
            $data['descricao'],
            $data['preco'],
    ...
    public static function update($db, $id, $data) {
        $stmt = $db->prepare('UPDATE produtos SET nome=?, descricao=?, preco=?, imagem=?, categoria_id=?, destaque=?, tipo_estoque=?, estoque=? WHERE id=?');
        $stmt->execute([
            $data['nome'],
            $data['descricao'],
            $data['preco'],
